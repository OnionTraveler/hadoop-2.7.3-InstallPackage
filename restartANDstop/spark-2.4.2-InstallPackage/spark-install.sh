...
cd /tmp; tar zxvf /tmp/spark-2.4.2-bin-hadoop2.7.tgz && rm /tmp/spark-2.4.2-bin-hadoop2.7.tgz; mv spark-2.4.2-bin-hadoop2.7 /opt

cat >> ~/restartANDstop/spark_StartStop/stop-all4Spark.sh << ONION
source /etc/profile
source /opt/spark-2.4.2-bin-hadoop2.7/sbin/stop-all.sh
ONION

cat >> ~/restartANDstop/spark_StartStop/start-all4Spark.sh << ONION
source /etc/profile
source /opt/spark-2.4.2-bin-hadoop2.7/sbin/start-all.sh
ONION
